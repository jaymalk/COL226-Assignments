...
let rec gettype g e : exptype = match e with
  (* Basics *)
  | Var(st) -> (try item_from_list st g with NotFound -> raise Wrong_Type)
  | N(x) -> (Tint)
  | B(b) -> (Tbool)
  ...
  | Project((i, n), e) -> (match e with Tuple(n, el) -> ( gettype g (List.nth el (i-1))) | Var(x) -> (match item_from_list x g with Ttuple(l) -> List.nth l (i-1) | _ -> raise Wrong_Type) | _ -> raise Wrong_Type)
  (* Local definition *)
